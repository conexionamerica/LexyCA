...
import { X, Sparkles, User, Mail, Lock, ShieldCheck, ArrowRight, AlertCircle, LogIn, UserPlus } from 'lucide-react';
...
export default function BookingAuthModal({ isOpen, onClose, tutor, onSuccessNavigate }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto animate-fade-in">
      <div className="relative w-full max-w-lg bg-slate-900 border border-cyan-500/40 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6 glow-cyan my-8">
        ...
        <div className="bg-cyan-500/10 border border-cyan-500/30 rounded-2xl p-4 flex items-start gap-3">
          ...
          <div className="text-xs text-slate-300 leading-relaxed">
            ...
            Para concluir o agendamento e utilizar a carteira <strong className="text-cyan-300">Lexy Pay</strong>, é necessário criar uma conta gratuita ou fazer login.
          </div>
        </div>
        ...
        {activeTab === 'signup' && (
          <form onSubmit={handleRegisterSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-gradient-to-r from-cyan-500 to-sky-400 hover:from-cyan-400 text-slate-950 font-black text-xs py-3.5 px-4 rounded-xl shadow-lg shadow-cyan-500/25 flex items-center justify-center gap-2 transition-all disabled:opacity-50 mt-2"
            >
