...
import { 
  ...
  ChevronLeft, Grid3x3, List, RefreshCw, Search, Camera, Upload, Lock, Globe, Mail, Zap
} from 'lucide-react';
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-4 space-y-4 animate-fade-in-up">
      ...
      <div className="mt-4">
            ...
            <div className="glass-panel rounded-3xl p-6 border border-cyan-500/30 shadow-2xl space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="bg-slate-950 border border-slate-800 p-3.5 rounded-2xl text-center">
                  ...
                  <span className="text-xl font-black text-amber-400">
                    {tutor?.rating && tutor?.reviewCount > 0 ? `${Number(tutor.rating).toFixed(1)} ★` : (totalLessons > 0 ? '5.0 ★' : 'Novo')}
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-col space-y-3">
                {myStudentsList.length === 0 ? (
                  <div className="bg-slate-950/80 border border-slate-800 rounded-2xl p-8 text-center space-y-2">
                    <Users className="w-8 h-8 text-slate-600 mx-auto opacity-60" />
                    <h4 className="text-sm font-bold text-white">Nenhum aluno vinculado ainda</h4>
                    <p className="text-xs text-slate-400 max-w-sm mx-auto">
                      Assim que alunos agendarem aulas com você, eles aparecerão nesta lista.
                    </p>
                  </div>
                ) : (
                      ...
                      <div className="flex items-center gap-3.5">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2.5 text-xs text-slate-400 mt-1 flex-wrap">
                            <span>{st.lessonsCount || 0} aula(s) contratada(s)</span>
                            <span className="text-slate-700">•</span>
                            <span>Status: <strong className="text-emerald-400 font-semibold">Ativo</strong></span>
                            {st.phone && (
                              <>
                                <span className="text-slate-700">•</span>
                                <span className="font-mono text-slate-400">📞 {st.phone}</span>
                              </>
                            )}
                          </div>
                        </div>
        ...
        {/* ABA: LEXY SPACE (AULAS AO VIVO E SALA VIRTUAL DO PROFESSOR) */}
        {activeTab === 'lexy-space' && (
          <div className="space-y-5 animate-fade-in-up max-w-5xl mx-auto">
            
            {/* HEADER DESTACADO DO LEXY SPACE DO PROFESSOR */}
            <div className="bg-gradient-to-r from-slate-900 via-amber-950/40 to-slate-900 border border-amber-500/30 rounded-2xl p-5 sm:p-6 shadow-2xl relative overflow-hidden flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="space-y-1.5 z-10">
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-300 text-[11px] font-black uppercase tracking-wider">
                  ...
                  <span>Lexy Space • Central do Tutor ao Vivo</span>
                </div>
                <h2 className="text-xl sm:text-2xl font-black text-white">
                  Minhas Aulas Particulares & Sala Virtual
                </h2>
                <p className="text-xs text-slate-300 max-w-xl leading-relaxed">
                  Entre na sua videochamada ao vivo com seu aluno, compartilhe a lousa virtual, anote materiais didáticos e acompanhe o tempo de aula em tempo real.
                </p>
              </div>

              <button
                onClick={() => setActiveTab('agenda')}
                className="z-10 bg-slate-900 hover:bg-slate-800 text-amber-400 border border-amber-500/30 hover:border-amber-500/60 font-bold text-xs px-4 py-2.5 rounded-xl transition-all cursor-pointer flex items-center gap-2 shrink-0 shadow-md"
              >
            ...
            <div className="bg-slate-900/60 backdrop-blur-md border border-slate-800/80 rounded-2xl p-2 sm:p-3 flex items-center gap-2 flex-wrap shadow-lg">
              ...
              <button
                type="button"
                onClick={() => setTeacherLexyDateFilter('hoje')}
                className={`px-4 py-2 rounded-xl text-xs font-black transition-all cursor-pointer flex items-center gap-1.5 ${
                  teacherLexyDateFilter === 'hoje'
              ...
              >
                <span>⚡ Hoje</span>
                <span className={`text-[10px] px-1.5 py-0.2 rounded-full ${teacherLexyDateFilter === 'hoje' ? 'bg-slate-950 text-amber-300' : 'bg-slate-900 text-slate-400'}`}>
                  {countTeacherHoje}
                </span>
              </button>
              ...
              <button
                type="button"
                onClick={() => setTeacherLexyDateFilter('amanha')}
                className={`px-4 py-2 rounded-xl text-xs font-black transition-all cursor-pointer flex items-center gap-1.5 ${
                  teacherLexyDateFilter === 'amanha'
                    ? 'bg-gradient-to-r from-cyan-500 to-cyan-600 text-slate-950 shadow-md shadow-cyan-500/20 scale-105'
                    : 'bg-slate-950 text-slate-300 hover:bg-slate-800 hover:text-white border border-slate-800'
                }`}
              >
                <span>🌅 Amanhã</span>
                <span className={`text-[10px] px-1.5 py-0.2 rounded-full ${teacherLexyDateFilter === 'amanha' ? 'bg-slate-950 text-cyan-300' : 'bg-slate-900 text-slate-400'}`}>
                  {countTeacherAmanha}
                </span>
              </button>
              ...
              <button
                type="button"
                onClick={() => setTeacherLexyDateFilter('todas')}
                className={`px-4 py-2 rounded-xl text-xs font-black transition-all cursor-pointer flex items-center gap-1.5 ${
                  teacherLexyDateFilter === 'todas'
              ...
              >
                ...
                <span className={`text-[10px] px-1.5 py-0.2 rounded-full ${teacherLexyDateFilter === 'todas' ? 'bg-slate-900 text-amber-400' : 'bg-slate-900 text-slate-400'}`}>
                  {countTeacherTodas}
                </span>
              </button>
            </div>
            ...
            {teacherLexyBookings.length === 0 ? (
              <div className="bg-slate-900/40 border border-slate-800/80 rounded-2xl p-8 text-center space-y-3">
                <Calendar className="w-10 h-10 text-slate-600 mx-auto" />
                <h3 className="text-base font-bold text-white">Nenhuma aula agendada encontrada ({teacherLexyDateFilter.toUpperCase()})</h3>
                <p className="text-xs text-slate-400 max-w-md mx-auto">
                  Alterne o filtro de data ou verifique sua agenda semanal.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-4">
                {teacherLexyBookings.map((b) => {
                  const dayCat = getTeacherBookingDayCategory(b.day, b.date);
                  return (
                    <div 
                      key={b.id} 
                    ...
                    >
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-slate-800/80 pb-3">
                        <div className="flex items-center gap-3">
                          <div className="w-11 h-11 rounded-2xl bg-amber-500/20 text-amber-400 font-black flex items-center justify-center border border-amber-500/40 shrink-0">
                            {(b.studentName || 'A').charAt(0).toUpperCase()}
                          </div>
                          <div>
                            <div className="flex items-center gap-2">
                              <h4 className="font-extrabold text-white text-sm sm:text-base">{b.studentName || 'Aluno Lexy'}</h4>
                              {dayCat === 'hoje' && (
                            ...
                            <p className="text-xs text-slate-400">{b.studentLevel || 'Nível Intermediário'} • {b.studentEmail || 'Aluno Registrado'}</p>
                          </div>
                        </div>

                        <div className="flex items-center gap-2 shrink-0">
                          <span className="bg-amber-500/10 text-amber-300 font-mono text-[11px] font-bold px-2.5 py-1 rounded-lg border border-amber-500/20">
